...
class LicensePlateNoGenerator(object):
    ...
    def generate_license_plate_numbers(self, num):
        ...
        if self.plate_type in ['single_blue', 'single_yellow', 'double_yellow']:
        ...
        # 挂车车牌号规则（车架号）
            ...
            license_plate_numbers = self.generate_trailer_license_no(num)
        # 警车车牌号规则
    ...
    def generate_standard_license_no(self, num):
        ...
        for i, lic_no in enumerate(license_plate_numbers):
            ...
            # 字母数多于两个的，/*随机保留2个（这样会导致车牌中2位字母数的车牌比较多）*/  改成 随机保留0-2个
            if len(alphabet_loc) > 2:
                allow_num = np.random.randint(3)
                allow = np.random.choice(a=alphabet_loc, size=allow_num, replace=False)
                if allow_num > 0:
                    for i in range(allow_num):
                        alphabet_loc.remove(allow[i])
    ...
    def generate_small_new_energy_license_no(self, num):
        ...
        for char_index in range(8):
            ...
            if char_index == 0:
                ...
                char_range = ['D', 'A', 'B', 'F', 'G']  # 小型新能源第3位为纯电D,A,B或非纯电F,G
            elif char_index == 3:
                # 该位规则符合取值范围
                pass
            else:
                char_range = LicensePlateNoGenerator.numerals  # 小型新能源后4位必须用数字
    ...
    def generate_big_new_energy_license_no(self, num):
        ...
        for char_index in range(8):
            ...
            if char_index == 0:
                ...
                char_range = LicensePlateNoGenerator.numerals  # 大型新能源前3-7位必须用数字
    ...
    def generate_trailer_license_no(self, num):
        """ 生成挂车标准规则车牌号（车架号）
        :param num: 生成车牌的数量
        :return: 生成的车牌号列表
        """
